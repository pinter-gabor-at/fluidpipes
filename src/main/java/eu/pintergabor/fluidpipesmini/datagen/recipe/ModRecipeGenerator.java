...
public final class ModRecipeGenerator extends RecipeProvider {
	...
	private void registerWoodenPipeRecipes() {
		createPipeRecipe(ItemTags.PLANKS,
			ModFluidBlocks.WOOD_PIPE.getBlock(), 6)
			.save(output);
		createPipeRecipe(Items.BAMBOO_PLANKS,
			ModFluidBlocks.BAMBOO_PIPE.getBlock(), 6)
			.save(output);
	}
	...
	private void registerWoodenFittingRecipes() {
		IntStream.range(0, ModFluidBlocks.WOODEN_PIPES.length).forEach(i ->
			createFittingRecipe(ModFluidBlocks.WOODEN_PIPES[i].getBlock(),
				ModFluidBlocks.WOODEN_FITTINGS[i].getBlock(), 8)
				.save(output));
	}
	...
	private void registerStonePipeRecipes() {
		createPipeRecipe(Items.STONE,
			ModFluidBlocks.STONE_PIPE.getBlock(), 6)
			.save(output);
		createPipeRecipe(Items.STONE,
			ModFluidBlocks.STONE_PIPE.getBlock(), 6)
			.save(output, RecipeBuilder.getDefaultRecipeId(ModFluidBlocks.STONE_PIPE.getBlock()) + "2");
		createPipeRecipe(Items.OBSIDIAN,
			ModFluidBlocks.OBSIDIAN_PIPE.getBlock(), 6)
			.save(output);
	}
	...
	private void registerStoneFittingRecipes() {
		IntStream.range(0, ModFluidBlocks.STONE_PIPES.length).forEach(i ->
			createFittingRecipe(ModFluidBlocks.STONE_PIPES[i].getBlock(),
				ModFluidBlocks.STONE_FITTINGS[i].getBlock(), 8)
				.save(output));
	}
